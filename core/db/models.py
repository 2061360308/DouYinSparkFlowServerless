...
连接地址通过环境变量 DATABASE_URL 指定，默认使用本地 SQLite 文件 db.sqlite3。
    ...
    # 生产环境使用 PostgreSQL
    DATABASE_URL=postgres://user:password@127.0.0.1:5432/dbname python -m core.db.init_db
"""
...
TORTOISE_ORM = {
    ...
    "apps": {
        "models": {
            ...
            "models": ["core.db.models", "core.db.domain_models", "core.task.models"],
...
# 需要新增配置时，在此追加键与默认值，再执行一次 python -m core.db.init_db 即可。
