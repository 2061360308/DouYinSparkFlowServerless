...
用法（需先建表：``python -m db``）：
    # 创建管理员（省略密码则自动生成并打印）
    python -m console create-admin <用户名> [--password <密码>]
    # 管理员已存在但忘记密码：重置(并确保为管理员)
    python -m console create-admin <用户名> --password <新密码> --reset
    # 重置任意已存在用户的密码(忘记密码自救; 省略 --password 则随机生成并打印)
    python -m console set-password <用户名> [--password <新密码>]
...
from db.console_models import User, WebSession
        ...
        try:
            ...
            if not reset:
                print(
                    ...
                    f"python -m console set-password {username} --password <新密码>",
                    file=sys.stderr,
                )
